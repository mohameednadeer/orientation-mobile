...
class _HomeFeedScreenState extends State<HomeFeedScreen>
  ...
  bool _isFreeProjectsLoading = true;
  ...
  Future<void> _loadVideoForPage(int index) async {
    ...
    try {
      ...
      if (heroVideoUrl.isEmpty) {
        print(
            '⚠️ No hero video URL for project at index $index: ${project.title}');
        return;
      }
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        // Map isFeatured to true for all items from getFeaturedProjects
        ...
        // Fetch Free projects from NestJS backend in background (non-blocking)
        ProjectService.getFreeProjects().then((freeProj) {
          if (mounted) {
            setState(() {
    ...
    Future.delayed(const Duration(milliseconds: 100), () {
      ...
      if (context != null && mounted) {
        Scrollable.ensureVisible(
          ...
          curve: Curves.easeInOut,
          alignment:
              0.1, // Scroll to show the section near the top with some padding
        );
      }
    });
  }
          ...
          children: [
            ...
            PageView.builder(
              ...
              onPageChanged: (index) {
                ...
                if (index + 1 < _featuredProjects.length &&
                    !_videoControllers.containsKey(index + 1)) {
                  _loadVideoForPage(index + 1);
                }
                if (index + 2 < _featuredProjects.length &&
  ...
  Widget _buildFeaturedCard(
    ...
    final bool isSwipeVisiblePage = _isSwipingFeatured &&
        (index == _swipeFromIndex || index == _swipeToIndex);
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            padding: EdgeInsets.only(
                right: index < _latestProjects.length - 1 ? 12 : 0),
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            padding: EdgeInsets.only(
                right: index < _continueWatching.length - 1 ? 12 : 0),
  ...
  Widget _buildCardImage({
    ...
    final imageAsset =
        (project.isAsset && project.projectThumbnailUrl.startsWith('assets/'))
            ? (project.projectThumbnailUrl.isNotEmpty
                ? project.projectThumbnailUrl
                : (fallbackImage != null && fallbackImage.startsWith('assets/')
                    ? fallbackImage
                    : null))
            : null;

    print(
        '🖼️ [$cardType] Building image for: ${project.title} (id: ${project.id})');
    print('   projectThumbnailUrl: "${project.projectThumbnailUrl}"');
    print('   image: "${project.image}"');
    print(
        '   isAsset: ${project.isAsset}, startsWith assets/: ${project.projectThumbnailUrl.startsWith('assets/')}');
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            ...
            child: RankedProjectCard(
              rank: index + 1,
              title: project.title,
              imageAsset: (project.isAsset &&
                      project.projectThumbnailUrl.startsWith('assets/'))
                  ? (project.projectThumbnailUrl.isNotEmpty
                      ? project.projectThumbnailUrl
                      : project.image)
                  : null,
              imageUrl: (project.isAsset &&
                      project.projectThumbnailUrl.startsWith('assets/'))
                  ? null
                  : (project.projectThumbnailUrl.isNotEmpty
                      ? project.projectThumbnailUrl
              ...
              onTap: () async {
                ...
                if (result == true) {
                  print(
                      '🔄 Episode was watched, refreshing continue watching...');
                  await _refreshContinueWatching();
                  print('✅ Continue watching refreshed after watching episode');
                }
              },
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            padding: EdgeInsets.only(
                right: index < _newCairoProjects.length - 1 ? 12 : 0),
            child: _buildLargeProjectCard(
              project: project,
            ),
          );
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            padding: EdgeInsets.only(
                right: index < _octoberProjects.length - 1 ? 12 : 0),
            child: _buildLargeProjectCard(
              project: project,
            ),
          );
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(
            padding: EdgeInsets.only(
                right: index < _upcomingProjects.length - 1 ? 12 : 0),
            child: ProjectCard(
              title: project.title,
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return _buildAreaChip(area,
              onTap: () => _navigateToAreaProjects(area));
        },
      ),
    );
  }
